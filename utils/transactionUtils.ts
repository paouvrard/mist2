  ...
  try {
    ...
    if (isEIP1559Supported) {
      ...
      if (populatedTx.gasPrice === undefined) {
        try {
          ...
          // Add a small buffer (5%) to the gas price to ensure transaction success
          populatedTx.gasPrice = (gasPrice * 105n) / 100n;
        } catch (error) {
          console.warn('Failed to get gas price:', error);
          // Provide a conservative default if estimation fails
          populatedTx.gasPrice = 20000000000n; // 20 Gwei
        }
      }
    ...
    if (populatedTx.gas === undefined) {
      try {
        ...
        // Provide a conservative default gas limit if estimation fails
        populatedTx.gas = 300000n;
      }
    }
    
    return populatedTx as Partial<Transaction>;
  } catch (error) {
    console.error('Error populating transaction:', error);
    return transaction; // Return original transaction if something fails
  }
}
